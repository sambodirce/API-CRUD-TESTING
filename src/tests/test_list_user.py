...
def test_should_return_all_users_details_when_pass_a_valid_user_id():
...
def test_should_return_all_existent_users_details():
    # Create and get user id
...
def test_should_return_the_correct_attribute_when_list_all_existent_users_details():
    # Create and get user id
    response = requests.get(f'{URL_REQRES}/users/{EMPTY_VAR}')
    assert response.status_code == 200
    assert type(response.json()["data"]) is list
...
def test_should_not_return_users_details_when_pass_a_sequence_of_space_user_id():
    # Create and get user id
    response = requests.get(f'{URL_REQRES}/users/{SEQUENCE_OF_SPACE_VAR}')
    assert response.status_code == 400


def test_should_not_return_users_details_when_pass_non_existent_user_id():
    # Create and get user id
